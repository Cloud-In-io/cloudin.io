...
{
    public static IApplicationBuilder MapUpload(
        this IApplicationBuilder app,
        string path = "/api/upload",
        string storePath = @"./tmp/uploads/"
    ) =>
        app.UseTus(
            httpContext =>
                    ...
                    Events = new Events
                    {
                            ...
                            try
                            {
                                ...
                                if (
                                    ...
                                    throw new ArgumentException(
                                        "the token parameter was not provide or is invalid."
                                    );

                                TokenHelper.ValidateToken<IUploadPayload>(
                                    token.ToString(),
                                ...
                                if (!ctx.Metadata.ContainsKey("contentType"))
                                    throw new ArgumentException(
                                        "contentType metadata must be specified."
                                    );
                            }
                        ...
                        OnFileCompleteAsync = async (ctx) =>
                        {
                            var settings = httpContext.RequestServices
                                ...
                                .Value;
                            var fileService =
                                httpContext.RequestServices.GetRequiredService<IFileService>();

                            var hasToken = ctx.HttpContext.Request.Query.TryGetValue(
                                "token",
                                out var token
                            );
                            var payload = TokenHelper.ValidateToken<IUploadPayload>(
                                token.ToString(),
                                secret: settings.UploadJWTSecret
                            );
                            ...
                            await terminationStore.DeleteFileAsync(
                                ctx.FileId,
